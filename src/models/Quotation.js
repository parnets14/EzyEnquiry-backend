...
async function getNextQuotationNo(company_id) {
  const last = await QuotationModel
    .findOne({ company_id, quotation_no: /^QT-/ })
    .sort({ created_at: -1 })
    .lean()
  if (!last?.quotation_no) return 'QT-0001'
  const num = parseInt(last.quotation_no.split('-')[1], 10) || 0
  return `QT-${String(num + 1).padStart(4, '0')}`
}

module.exports = { QuotationModel, getNextQuotationNo }
